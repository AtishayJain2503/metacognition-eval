"""
nemo_eval.datasets.putnam
=========================
PutnamBench benchmark dataset loader (50 competition-grade mathematical tasks).
"""
...
class PutnamBenchLoader(BaseDatasetLoader):
    """
    Loader for the PutnamBench mathematical competition benchmark.
    
    Provides 50 competition-grade problems from the William Lowell Putnam Mathematical Competition
    across core advanced mathematics disciplines: Real Analysis, Abstract Algebra, Linear Algebra,
    Number Theory, Combinatorics, Geometry, and Calculus.
    """
    ...
    def _load_raw_tasks(self) -> List[BenchmarkTask]:
        ...
        if fixture_file.exists():
            ...
            from nemo_eval.datasets.fixtures.generate_fixtures import generate_putnam_fixtures
            records = generate_putnam_fixtures()
            for r in records:
    ...
    def load_tasks(self, limit: Optional[int] = None, use_1000: Optional[bool] = None) -> List[BenchmarkTask]:
        ...
        if self.category:
            cat_clean = self.category.lower().strip().replace(" ", "_")
            filtered = [
                t for t in filtered
                if t.subdiscipline.lower().strip().replace(" ", "_") == cat_clean
                or str(t.metadata.get("category", "")).lower().strip().replace(" ", "_") == cat_clean
            ]
    ...
    def get_task(self, task_id: str) -> BenchmarkTask:
        ...
        for t in all_tasks:
            if t.task_id == task_id:
                return t
        raise KeyError(f"Task '{task_id}' not found in PutnamBench dataset.")

    def get_categories(self) -> List[str]:
        """Return the list of Putnam mathematical categories."""
